...
def build_default_bead_rows() -> list[dict[str, str]]:
    return [dict(row) for row in DEFAULT_BEAD_ROWS]
...
def build_bead_rows_from_mesf_values(mesf_values: Any) -> list[dict[str, str]]:
    ...
    for raw_part in raw_parts:
        ...
        parsed_value = casting.as_float(raw_part)
...
def resolve_bead_rows_from_runtime_store(runtime_config_data: Any) -> list[dict[str, str]]:
    runtime_config = RuntimeConfig.from_dict(
        runtime_config_data if isinstance(runtime_config_data, dict) else None
    )
    mesf_values = runtime_config.get_path("calibration.mesf_values", default=[])
    return build_bead_rows_from_mesf_values(mesf_values)
...
def rebuild_calibration_graph(
    *,
    ...
    if not stored_figure:
        ...
        return _make_info_figure(empty_message)
    ...
    try:
        ...
        return _make_info_figure(failure_message)
...
def add_empty_row(
    *,
    ...
    next_rows.append({str(column["id"]): "" for column in (columns or [])})
    return next_rows
...
def extract_xy_from_table(table_data: list[dict[str, Any]] | None) -> ExtractedCalibrationPoints:
    ...
    for row in table_data or []:
        intensity_calibrated_units = casting.as_float(row.get("col1"))
        intensity_au = casting.as_float(row.get("col2"))
...
def fit_log10_calibration(
    *,
    ...
    if intensity_au.size < 2:
        raise ValueError("Need at least 2 positive finite bead points for log10 fit.")
...
def build_calibration_payload(
    *,
    ...
    return {
        ...
        "gating_threshold": casting.as_float(scattering_threshold),
...
def build_calibration_figure(
    *,
    ...
    x_log10 = np.asarray(x_log10, dtype=float)
...
def compute_valid_event_count_for_preview(
    *,
    ...
    return int(calibrated_intensity_units.size)
...
def run_calibration_workflow(
    *,
    ...
    if detector_column:
        ...
        logger.debug("No fluorescence detector selected. Returning calibration fit without detector preview.")
