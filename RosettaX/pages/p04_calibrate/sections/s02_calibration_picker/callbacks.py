...
class CalibrationPickerCallbacks:
    ...
    def _register_target_model_preset_callback(self) -> None:
        ...
        def apply_target_model_preset(
            ...
            logger.debug(
                "apply_target_model_preset called with target_model_preset=%r selected_calibration_summary=%r",
                target_model_preset,
                selected_calibration_summary,
            )

            preset_wavelength_nm = None
            scattering_calibration_summary = services.get_primary_scattering_calibration_summary(
                selected_calibration_summary,
            )

            if isinstance(scattering_calibration_summary, dict):
                ...
                if isinstance(calibration_payload, dict) and calibration_payload:
                    ...
                    preset_wavelength_nm = parse_optical_geometry_from_calibration_standard_parameters(
                        calibration_standard_parameters,
                    ).get(
                        "wavelength_nm",
                    )

            preset = resolve_scattering_target_model_preset(
                target_model_preset,
                wavelength_nm=preset_wavelength_nm,
            )

            logger.debug(
                "apply_target_model_preset resolved preset=%r preset_wavelength_nm=%r",
                preset,
                preset_wavelength_nm,
            )
