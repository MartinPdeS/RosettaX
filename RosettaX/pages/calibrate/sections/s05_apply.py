...
class ApplySection:
    ...
    def register_callbacks(self) -> None:
        ...
        def apply_and_export_calibration(
            ...
            if not resolved_uploaded_fcs_path:
                return (
                    "Upload an input FCS file first.",
                    dash.no_update,
                )
            ...
            try:
                ...
                with FCSFile(str(resolved_uploaded_fcs_path), writable=False) as input_fcs_file:
                ...
                download_filename = self._build_export_filename(
                    uploaded_fcs_path=resolved_uploaded_fcs_path,
            ...
            logger.debug(
                "Calibration export succeeded with download_filename=%r exported_columns=%r",
                download_filename,
            ...
            return (
                (
                    ...
                    f'and prepared export with columns: {", ".join(final_export_columns)}.'
                ),
    ...
    def _apply_calibration_to_series(
        *,
        ...
        raise ValueError(
            "Unsupported calibration payload format. "
            'Expected "slope"/"intercept" or "scale"/"offset".'
        )
    ...
    def _build_export_filename(
        *,
        ...
        safe_target_channel = str(target_channel).replace("/", "_").replace("\\", "_").replace(" ", "_")
