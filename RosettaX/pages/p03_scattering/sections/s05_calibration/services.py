...
def build_instrument_response_figure(
    *,
    ...
    if measured_peak_positions.size == 0:
        return plottings._make_info_figure("No valid instrument response data available.")
    ...
    if x_max == x_min:
        fit_x_values = np.asarray([x_min, x_min * 1.01], dtype=float)
    else:
        fit_x_values = np.linspace(x_min, x_max, 200)
...
def build_solid_sphere_dense_simulated_coupling_curve(
    *,
    ...
    """
    Build a dense solid sphere Mie relation for the right graph.
    """
    particle_diameters_nm = np.asarray(particle_diameters_nm, dtype=float).reshape(-1)
    ...
    dense_particle_diameters_nm = np.linspace(diameter_min, diameter_max, int(simulated_curve_point_count))

    dense_modeled_coupling_result = BackEnd.compute_modeled_coupling_from_diameters(
        particle_diameters_nm=dense_particle_diameters_nm,
        ...
        detector_cache_numerical_aperture=optical_parameters.modeling_detector_cache_numerical_aperture,
...
def build_core_shell_dense_simulated_coupling_curve(
    *,
    ...
    """
    Build a dense core shell Mie relation for the right graph.
    """
    core_diameters_nm = np.asarray(core_diameters_nm, dtype=float).reshape(-1)
    shell_thicknesses_nm = np.asarray(shell_thicknesses_nm, dtype=float).reshape(-1)

    valid_mask = (
        np.isfinite(core_diameters_nm)
        & np.isfinite(shell_thicknesses_nm)
        & (core_diameters_nm > 0.0)
        & (shell_thicknesses_nm > 0.0)
    )
    core_diameters_nm = core_diameters_nm[valid_mask]
    shell_thicknesses_nm = shell_thicknesses_nm[valid_mask]

    if core_diameters_nm.size == 0:
        return np.asarray([], dtype=float), np.asarray([], dtype=float)

    if optical_parameters.core_refractive_index is None:
        raise ValueError("core_refractive_index is required for Core/Shell Sphere calibration.")

    if optical_parameters.shell_refractive_index is None:
        raise ValueError("shell_refractive_index is required for Core/Shell Sphere calibration.")

    outer_diameters_nm = core_diameters_nm + 2.0 * shell_thicknesses_nm
    outer_diameter_min = float(np.min(outer_diameters_nm))
    outer_diameter_max = float(np.max(outer_diameters_nm))

    if outer_diameter_min <= 0.0 or outer_diameter_max <= 0.0:
        return np.asarray([], dtype=float), np.asarray([], dtype=float)

    if outer_diameter_max == outer_diameter_min:
        outer_diameter_max = outer_diameter_min * 1.01

    dense_outer_diameters_nm = np.linspace(outer_diameter_min, outer_diameter_max, int(simulated_curve_point_count))
    fixed_shell_thickness_nm = float(np.median(shell_thicknesses_nm))
    dense_core_diameters_nm = dense_outer_diameters_nm - 2.0 * fixed_shell_thickness_nm

    valid_dense_mask = np.isfinite(dense_core_diameters_nm) & (dense_core_diameters_nm > 0.0)
    dense_core_diameters_nm = dense_core_diameters_nm[valid_dense_mask]
    dense_shell_thicknesses_nm = np.full(dense_core_diameters_nm.size, fixed_shell_thickness_nm, dtype=float)

    if dense_core_diameters_nm.size == 0:
        return np.asarray([], dtype=float), np.asarray([], dtype=float)

    dense_modeled_coupling_result = BackEnd.compute_modeled_coupling_from_core_shell_dimensions(
        core_diameters_nm=dense_core_diameters_nm,
        shell_thicknesses_nm=dense_shell_thicknesses_nm,
        wavelength_nm=optical_parameters.wavelength_nm,
        source_numerical_aperture=optical_parameters.source_numerical_aperture,
        optical_power_watt=optical_parameters.optical_power_watt,
        detector_numerical_aperture=optical_parameters.detector_numerical_aperture,
        medium_refractive_index=optical_parameters.medium_refractive_index,
        core_refractive_index=float(optical_parameters.core_refractive_index),
        shell_refractive_index=float(optical_parameters.shell_refractive_index),
        detector_cache_numerical_aperture=optical_parameters.modeling_detector_cache_numerical_aperture,
        detector_phi_offset_degree=optical_parameters.detector_phi_angle_degree,
        detector_gamma_offset_degree=optical_parameters.detector_gamma_angle_degree,
        polarization_angle_degree=optical_parameters.polarization_angle_degree,
...
def build_calibration_standard_mie_relation_figure(
    *,
    ...
    if point_x_values.size == 0 and line_x_values.size == 0:
        return plottings._make_info_figure("No calibration standard Mie relation data available.")
    ...
    try:
        ...
        if resolved_mie_model == "Core/Shell Sphere":
            ...
            for row in current_table_rows or []:
                ...
                if core_diameter_nm <= 0.0 or shell_thickness_nm <= 0.0 or expected_coupling <= 0.0:
                    continue

                core_diameters_nm.append(core_diameter_nm)
                shell_thicknesses_nm.append(shell_thickness_nm)
                outer_diameters_nm.append(core_diameter_nm + 2.0 * shell_thickness_nm)
            ...
            figure = build_calibration_standard_mie_relation_figure(
                ...
                expected_coupling_values=np.asarray(expected_coupling_values, dtype=float),
        ...
        return plottings._make_info_figure("Failed to render calibration standard Mie relation graph.").to_dict()
...
def build_missing_input_result(
    ...
    """
    left_figure = plottings._make_info_figure(message)
    right_figure = plottings._make_info_figure(message)
...
def run_solid_sphere_calibration(
    *,
    ...
    if parsed_sphere_rows.row_count < 2:
        ...
        right_figure = plottings._make_info_figure("You may need to click Compute Standard Coupling first.")
        return CalibrationResult(
            figure_store=left_figure.to_dict(),
            model_figure_store=right_figure.to_dict(),
            calibration_store=None,
    ...
    particle_diameters_nm = np.asarray(modeled_coupling_result.particle_diameters_nm, dtype=float).reshape(-1)
    expected_coupling_values = np.asarray(modeled_coupling_result.expected_coupling_values, dtype=float).reshape(-1)
    ...
    metadata = {
        "measured_channel": str(detector_column),
...
def run_core_shell_calibration(
    *,
    uploaded_fcs_path: str,
    detector_column: str,
    current_table_rows: list[dict[str, Any]],
    ...
    """
    Run core shell sphere scattering instrument response calibration.
    """
    if optical_parameters.core_refractive_index is None:
        raise ValueError("core_refractive_index is required for Core/Shell Sphere calibration.")
    ...
    modeled_coupling_result = scattering_backend.compute_modeled_coupling_from_core_shell_dimensions(
        core_diameters_nm=parsed_core_shell_rows.core_diameters_nm,
        shell_thicknesses_nm=parsed_core_shell_rows.shell_thicknesses_nm,
        wavelength_nm=optical_parameters.wavelength_nm,
        source_numerical_aperture=optical_parameters.source_numerical_aperture,
        optical_power_watt=optical_parameters.optical_power_watt,
        detector_numerical_aperture=optical_parameters.detector_numerical_aperture,
    ...
    outer_diameters_nm = np.asarray(modeled_coupling_result.particle_diameters_nm, dtype=float).reshape(-1)
    expected_coupling_values = np.asarray(modeled_coupling_result.expected_coupling_values, dtype=float).reshape(-1)
    ...
    build_result = scattering.build_core_shell_scattering_calibration_from_standard_data(
        ...
        expected_coupling_values=expected_coupling_values,
        dense_outer_diameters_nm=dense_outer_diameters_nm,
        dense_expected_coupling_values=dense_expected_coupling_values,
        optical_parameters=optical_parameters,
        metadata=metadata,
        force_zero_intercept=bool(force_zero_intercept),
    ...
    return CalibrationResult(
        ...
        intercept_out=f"{build_result.instrument_response.intercept:.6g}",
        r_squared_out=f"{build_result.instrument_response.r_squared:.6g}",
        apply_status="Core shell instrument response fitted successfully.",
    )
...
def run_scattering_calibration(
    *,
    ...
    """
    logger.debug(
        "run_scattering_calibration called with uploaded_fcs_path=%r detector_column=%r mie_model=%r table_row_count=%r force_zero_intercept=%r",
        uploaded_fcs_path,
        detector_column,
        mie_model,
    ...
    try:
        ...
        if resolved_mie_model == "Core/Shell Sphere":
            return run_core_shell_calibration(
                uploaded_fcs_path=str(uploaded_fcs_path),
                detector_column=str(detector_column),
                current_table_rows=current_table_rows,
                optical_parameters=optical_parameters,
        ...
        left_figure = plottings._make_info_figure("Instrument response fitting failed.")
        right_figure = plottings._make_info_figure("Calibration standard Mie relation failed.")
        return CalibrationResult(
            figure_store=left_figure.to_dict(),
            model_figure_store=right_figure.to_dict(),
            calibration_store=None,
