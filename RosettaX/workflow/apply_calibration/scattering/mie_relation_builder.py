...
def _optional_float(value: Any) -> float | None:
    """Parse an optional float value from a calibration parameter payload."""
    if value in (None, ""):
        return None
...
def resolve_detector_modeling_context(
    *,
    optical_geometry: dict[str, Any],
    medium_refractive_index: float,
) -> dict[str, Any]:
    """
    Resolve apply-time detector modeling inputs to match calibration behavior.

    This restores preset-driven effective geometry and detector angular weights
    when those assumptions were used during calibration.
    """
    preset_name = str(
        optical_geometry.get("detector_configuration_preset_name", "") or ""
    )

    current_detector_cache_numerical_aperture = float(
        optical_geometry["detector_cache_numerical_aperture"]
    )
    current_blocker_bar_numerical_aperture = float(
        optical_geometry["blocker_bar_numerical_aperture"]
    )

    resolved_effective_detector_cache_numerical_aperture = optical_geometry.get(
        "effective_detector_cache_numerical_aperture"
    )
    resolved_effective_blocker_bar_numerical_aperture = optical_geometry.get(
        "effective_blocker_bar_numerical_aperture"
    )

    if (
        resolved_effective_detector_cache_numerical_aperture is None
        or resolved_effective_blocker_bar_numerical_aperture is None
    ):
        (
            resolved_effective_detector_cache_numerical_aperture,
            resolved_effective_blocker_bar_numerical_aperture,
    ...
    detector_angular_weights = detector.resolve_detector_angular_weights(
        preset_name=preset_name,
        detector_sampling=int(optical_geometry["detector_sampling"]),
        current_detector_numerical_aperture=float(
            optical_geometry["detector_numerical_aperture"]
        ),
        current_detector_cache_numerical_aperture=current_detector_cache_numerical_aperture,
        current_blocker_bar_numerical_aperture=current_blocker_bar_numerical_aperture,
        current_detector_phi_angle_degree=float(optical_geometry["detector_phi_angle_degree"]),
        current_detector_gamma_angle_degree=float(optical_geometry["detector_gamma_angle_degree"]),
        current_medium_refractive_index=float(medium_refractive_index),
    )

    modeling_context = {
        "detector_configuration_preset_name": preset_name,
    ...
    logger.debug(
        "resolve_detector_modeling_context returning preset_name=%r "
        "effective_detector_cache_numerical_aperture=%r "
        "effective_blocker_bar_numerical_aperture=%r has_angular_weights=%r",
        modeling_context["detector_configuration_preset_name"],
        modeling_context["detector_cache_numerical_aperture"],
        modeling_context["blocker_bar_numerical_aperture"],
        modeling_context["detector_angular_weights"] is not None,
    )

    return modeling_context
...
def build_solid_sphere_target_mie_relation(
    *,
    ...
    detector_modeling_context = resolve_detector_modeling_context(
    ...
    modeled_coupling_result = BackEnd.compute_modeled_coupling_from_diameters(
        ...
        detector_cache_numerical_aperture=detector_modeling_context[
        ...
        detector_angular_weights=detector_modeling_context["detector_angular_weights"],
    )
    ...
    parameter_payload = build_target_relation_parameter_payload(
        ...
        optical_geometry=optical_geometry,
        detector_modeling_context=detector_modeling_context,
    )
...
def build_target_relation_parameter_payload(
    *,
    ...
    optical_geometry: dict[str, Any],
    detector_modeling_context: dict[str, Any],
) -> dict[str, Any]:
    """
    ...
    parameter_payload = scattering.build_mie_parameter_payload(
        ...
        blocker_bar_numerical_aperture=detector_modeling_context[
    ...
    parameter_payload.update(
        {
            ...
            "source_numerical_aperture": optical_geometry["source_numerical_aperture"],
            "polarization_angle_degree": optical_geometry["polarization_angle_degree"],
            "detector_configuration_preset_name": detector_modeling_context[
                "detector_configuration_preset_name"
            ],
            "effective_detector_cache_numerical_aperture": detector_modeling_context[
                "detector_cache_numerical_aperture"
            ],
            "effective_blocker_bar_numerical_aperture": detector_modeling_context[
                "blocker_bar_numerical_aperture"
            ],
            "detector_has_angular_weights": detector_modeling_context[
...
def parse_optical_geometry_from_calibration_standard_parameters(
    ...
    optical_geometry = {
        ...
        "effective_detector_cache_numerical_aperture": _optional_float(
            calibration_standard_parameters.get(
                "effective_detector_cache_numerical_aperture"
            )
        ),
        "effective_blocker_bar_numerical_aperture": _optional_float(
            calibration_standard_parameters.get(
                "effective_blocker_bar_numerical_aperture"
            )
        ),
