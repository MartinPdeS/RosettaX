...
class Test_RosettaPeakScript:
    def test_settings_keep_only_process_specific_rosetta_toggles(self) -> None:
        ...
        saturation_guard_setting = next(
            setting
            for setting in settings
            if setting.get("name") == "saturation_guard_fraction"
        )
        saturation_guard_enabled_setting = next(
            setting
            for setting in settings
            if setting.get("name") == "saturation_guard_enabled"
        )
    ...
    def test_run_automatic_action_applies_metadata_saturation_guard_fraction(
        self,
        monkeypatch,
    ) -> None:
        scattering = np.concatenate(
            [
                np.full(35, 1_200_000.0, dtype=float),
                np.linspace(2000.0, 15000.0, 300, dtype=float),
            ]
        )
        fluorescence = np.concatenate(
            [
                np.full(35, 500.0, dtype=float),
                np.linspace(120.0, 900.0, 300, dtype=float),
            ]
        )
        ...
        monkeypatch.setattr(
            ...
            lambda **kwargs: (
                {
                    "all_peaks": [
                        {"mean": 120.0, "std": 8.0, "count": 300, "validated": True, "rejection_reasons": []},
                        {"mean": 700.0, "std": 35.0, "count": 90, "validated": True, "rejection_reasons": []},
                    ],
                    "validated_peaks": [
                        {"mean": 120.0, "std": 8.0, "count": 300, "validated": True, "rejection_reasons": []},
                        {"mean": 700.0, "std": 35.0, "count": 90, "validated": True, "rejection_reasons": []},
                ...
                if float(np.max(np.asarray(kwargs["values"], dtype=float))) < 10_000.0
                else {
                    "all_peaks": [
                        {"mean": 700_000.0, "std": 30_000.0, "count": 120, "validated": True, "rejection_reasons": []},
                    ],
                    "validated_peaks": [
                        {"mean": 700_000.0, "std": 30_000.0, "count": 120, "validated": True, "rejection_reasons": []},
                    ],
        ...
        monkeypatch.setattr(
            rosetta_mix,
            "select_primary_validated_peak",
            lambda **kwargs: {"mean": 700_000.0, "std": 20_000.0, "count": 80},
        )
        monkeypatch.setattr(
            rosetta_mix,
            "build_peak_display_points",
            lambda **kwargs: [
                {"x": 900_000.0, "y": 130.0, "kind": "non_fluorescent"},
                {"x": 1_200_000.0, "y": 130.0, "kind": "non_fluorescent"},
            ],
        )
        monkeypatch.setattr(
            rosetta_mix,
            "resolve_detector_channel_saturation_value_from_metadata",
            lambda **kwargs: 4_000_000.0,
        )
        ...
        result = process.run_automatic_action(
            ...
            max_events_for_analysis=10000,
            process_settings={
                "saturation_guard_fraction": 0.25,
                "filter_edge_artifacts": False,
            },
        )

        plot_x_values = result.peak_lines_payload.get("plot_x_values", [])
        table_prefill_rows = result.table_prefill_rows or []
        table_peaks = [
            float(row["measured_peak_position"])
            for row in table_prefill_rows
            if isinstance(row, dict) and "measured_peak_position" in row
        ]

        assert plot_x_values
        assert len(plot_x_values) == len(scattering)
        assert len(table_peaks) == 1
        assert max(table_peaks) <= 1_000_000.0
        assert "saturation guard excluded" in str(result.status).lower()

    def test_saturation_guard_falls_back_to_data_when_metadata_is_suspicious(
        self,
        monkeypatch,
    ) -> None:
        monkeypatch.setattr(
        ...
        resolved_saturation_value, resolved_source = (
            rosetta_mix.resolve_scattering_saturation_value_for_guard(
                backend=SimpleNamespace(fcs_file_path="dummy.fcs"),
                detector_column="SSC-H",
                scattering_values=np.asarray([10.0, 4000.0, 4_000_000.0], dtype=float),
            )
        )

        assert resolved_saturation_value == 4_000_000.0
        assert resolved_source == "data-fallback"

    def test_saturation_guard_keeps_metadata_when_range_is_consistent(
        self,
        monkeypatch,
    ) -> None:
        monkeypatch.setattr(
        ...
        resolved_saturation_value, resolved_source = (
            rosetta_mix.resolve_scattering_saturation_value_for_guard(
                backend=SimpleNamespace(fcs_file_path="dummy.fcs"),
                detector_column="SSC-H",
                scattering_values=np.asarray([10.0, 4000.0, 3_900_000.0], dtype=float),
            )
        )

        assert resolved_saturation_value == 4_000_000.0
        assert resolved_source == "metadata"

    def test_run_automatic_action_skips_guard_when_toggle_disabled(
        self,
        monkeypatch,
    ) -> None:
        scattering = np.asarray([1000.0, 2000.0, 3000.0], dtype=float)
        fluorescence = np.asarray([100.0, 110.0, 120.0], dtype=float)
        ...
        monkeypatch.setattr(
            ...
            lambda **kwargs: (
                {
                    "all_peaks": [
                        {"mean": 110.0, "std": 5.0, "count": 100, "validated": True, "rejection_reasons": []},
                        {"mean": 500.0, "std": 30.0, "count": 80, "validated": True, "rejection_reasons": []},
                    ],
                    "validated_peaks": [
                        {"mean": 110.0, "std": 5.0, "count": 100, "validated": True, "rejection_reasons": []},
                        {"mean": 500.0, "std": 30.0, "count": 80, "validated": True, "rejection_reasons": []},
                ...
                if float(np.max(np.asarray(kwargs["values"], dtype=float))) < 1000.0
                else {
                    "all_peaks": [
                        {"mean": 2500.0, "std": 200.0, "count": 60, "validated": True, "rejection_reasons": []},
                    ],
                    "validated_peaks": [
                        {"mean": 2500.0, "std": 200.0, "count": 60, "validated": True, "rejection_reasons": []},
                    ],
        ...
        monkeypatch.setattr(
            rosetta_mix,
            "select_primary_validated_peak",
            lambda **kwargs: {"mean": 2500.0, "std": 120.0, "count": 40},
        )
        monkeypatch.setattr(
            rosetta_mix,
            "build_peak_display_points",
            lambda **kwargs: [
                {"x": 1500.0, "y": 110.0, "kind": "non_fluorescent"},
                {"x": 2800.0, "y": 110.0, "kind": "non_fluorescent"},
            ],
        )
        monkeypatch.setattr(
            rosetta_mix,
            "resolve_detector_channel_saturation_value_from_metadata",
            lambda **kwargs: 2000.0,
        )
        ...
        assert len(table_peaks) == 2
        assert max(table_peaks) == 2800.0
        assert "saturation guard excluded" not in str(result.status).lower()
