            ...
            """
{
    ...
    def test_loader_produces_same_presets_from_directory_as_json(self):
        """
        Verify that loading from split-file directory structure
        produces identical presets to the original JSON catalog.
        This ensures migration from JSON to directory layout is safe.
        """
        loader = DetectorPresetLoader()
        presets = loader.load_presets()

        # Verify all known detectors are loaded
        expected_detector_names = {
            "Agilent NovoCyte FSC",
            "Agilent NovoCyte SSC",
            "Apogee - Forward",
            "Apogee - Side",
            "BD FACSCanto II FSC",
            "BD FACSCanto II SSC",
            "Beckman Coulter CytoFLEX Fluorescence",
            "Beckman Coulter CytoFLEX SSC",
            "Cytek Aurora FSC",
            "Cytek Aurora SSC",
            "Generic detector",
            "Sony ID7000 FSC",
            "Sony ID7000 SSC",
            "Thermo Fisher Attune NxT FSC",
            "Thermo Fisher Attune NxT SSC",
            "nanoFCM NanoAnalyzer FSC",
            "nanoFCM NanoAnalyzer SSC",
        }

        loaded_names = set(presets.keys())
        assert loaded_names == expected_detector_names, (
            f"Loaded presets {loaded_names} do not match expected {expected_detector_names}"
        )

        # Verify each preset has required schema fields
        required_fields = {"name", "detector_numerical_aperture"}
        for preset_name, preset in presets.items():
            for field in required_fields:
                assert field in preset, f"Preset '{preset_name}' missing required field '{field}'"

        # Spot check: Verify specific preset content integrity
        apogee_forward = presets.get("Apogee - Forward")
        assert apogee_forward is not None
        assert apogee_forward["manufacturer"] == "Apogee"
        assert apogee_forward["detector_numerical_aperture"] == 1.2
        assert apogee_forward["detector_angular_weighting"]["mode"] == "split-separation-angle"
        assert apogee_forward["detector_angular_weighting"]["separation_angle_degree"] == 10.0
...
class Test_compute_model_for_rows:
    ...
    def test_resolve_detector_angular_weights_apogee_half_lens_split_presets(self):
        ...
        forward_eligible_indices = np.flatnonzero(
    ...
    def test_detector_preset_loader_normalizes_apogee_split_definition(self):
        ...
        assert apogee_side_preset['detector_angular_weighting'] == {
            ...
            'keep': 'positive',
            'separation_angle_degree': 10.0,
        }
        assert apogee_side_preset['alias'] == ['apogee']
        # Updated: presets are now loaded from vendor subdirectories, not from detector_definitions.json
        assert preset_path.name == 'apogee_side.json'
        assert preset_path.parent.name == 'apogee'
        assert preset_path.parent.parent.name == 'detector_definitions'
